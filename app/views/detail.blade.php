		...
		@if ($post->hasBuyOptions)
			<table class="buy-options" border="0" cellpadding="0" cellspacing="0">
			  <tbody>
				...
				<tr class="first">
					...
					<td>
						<div class="button">
							<a class="addtocart" data-type="small" data-name="{{ Config::get('prices.small.name') }}" data-pretty-price="{{ Config::get('prices.small.pretty_price') }}" data-price="{{ Config::get('prices.small.price') }}">
				...
				<tr>
				 <td class="print-size">{{ Config::get('prices.large.name') }}</td>
				  <td class="print-price">{{ Config::get('prices.large.pretty_price') }}</td>
					<td>
						<div class="button">
							<a class="addtocart" data-type="large" data-name="{{ Config::get('prices.large.name') }}" data-pretty-price="{{ Config::get('prices.large.pretty_price') }}" data-price="{{ Config::get('prices.large.price') }}">
								<div class="add">Add to Cart</div>
								<div class="added">Added!</div>
							</a>
						</div>
			...
			<table class="shipping" border="0" cellpadding="0" cellspacing="0">
			  <tbody>
				...
				<tr>
				  <td class="shipping-location strike">International</td>
				  <td class="shipping-price strike" align="right">+ $26.00</td>
				</tr>
			  </tbody>
			</table>
		@else
			  ...
			  var handler = StripeCheckout.configure({
				...
				token: function(token, args) {
				  ...
				  $.ajax({
					  url: 'buy/{{ $post->id }}',
					  type: 'post',
					  data: {token: token.id, email: token.email, printType: printType, desc: desc, name: "{{ $post->name }}"},
				...
				$('.addtocart').click(function() {
					...
					// desc = $(this).data('name') + ' Print (' + $(this).data('pretty-price') + ')';
					// handler.open({
					// 	name: '{{ $post->name }}',
					// 	description: desc,
